...
import { cn, on } from "@/renderer/lib/utils";
...
function Page() {
  ...
  const onCreateBlur =
    (key: ArrayKey) => (e: React.FocusEvent<HTMLInputElement>) => {
      let value = e.target.value;
      if (value === "") {
        return;
      }
      if (key === "extensions") {
        value = (value.split(".").pop() ?? value).toLowerCase();
      }
      e.target.value = "";
      if (rule[key].includes(value)) {
        return;
      }
      ...
      <ScrollArea className="flex-1">
        <main className="space-y-2 p-2">
          ...
          <Accordion className="space-y-2" type="multiple">
            <AccordionItem className="border-none" value="접두사">
              <Button
            ...
            <AccordionItem className="border-none" value="접미사">
              <Button
                className="h-12 justify-between"
                variant="secondary"
                asChild
            ...
            <AccordionItem className="border-none" value="확장자">
              <Button
                className="h-12 justify-between"
                variant="secondary"
                asChild
