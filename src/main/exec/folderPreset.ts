...
export async function applyFolderPreset(folderPresetId: string, path: string) {
  const raw = await db.select().from(folderPresetTable).where(eq(folderPresetTable.id, folderPresetId)).limit(1);
  ...
  const raw2 = await db.select().from(folderPresetTable).where(eq(folderPresetTable.parentId, folderPresetId));
